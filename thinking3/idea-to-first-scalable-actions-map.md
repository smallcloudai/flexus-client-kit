# Idea to First Scalable Actions

Last updated: 2026-02-24

## Scope

This map is for SMB contexts only:
- B2B
- B2C
- B2B2C
- B2B2B

The goal is to define stage structure from a raw idea to first scalable actions, at high enough level to stay clear, but concrete enough to decompose later into bots, experts, and skills.

## Level 0 Stage Map (Current Baseline)

1. Opportunity Framing
2. Problem and Segment Validation
3. Solution and Offer Hypothesis
4. MVP or Proof-of-Value
5. Early Commercial Validation
6. Fit Consolidation (PMF-level evidence)
7. GTM and Route-to-Market Fit
8. First Scalable Actions

## Operating Principles

- Stages are a map, not a strict gate system.
- Non-linear execution is allowed if boss orchestration requires it.
- Every stage is judged by evidence quality, not by activity volume.
- This map optimizes for SMB reliability and low operational overhead.

## Research Basis (External Synthesis)

This Level 1 decomposition is grounded in cross-framework synthesis, not one school:

- Customer Development (Steve Blank): Discovery -> Validation -> Creation -> Company Building.
- Lean Startup (Eric Ries): Build-Measure-Learn and hypothesis-driven iteration.
- Design Thinking (Stanford d.school): Empathize, Define, Ideate, Prototype, Test.
- Disciplined Entrepreneurship (Bill Aulet): segmentation, beachhead, TAM, and practical sequencing.
- JTBD / Outcome-Driven Innovation (Ulwick): job definition, outcome metrics, underserved needs.
- PMF and growth evidence: retention/cohort logic, PMF signal methods (including Sean Ellis test).
- GTM fit and unit economics: repeatable demand + sales motion + pricing/packaging consistency.
- Channel strategy: Bullseye traction model, AARRR lifecycle metrics, RTM architecture choices.
- Adoption transition: Crossing the Chasm (early adopters -> early majority motion change).
- Partner-led variants (B2B2C/B2B2B): partner onboarding/enablement/activation lifecycle.

## Level 1 Decomposition (One Step Deeper)

### Stage 1: Opportunity Framing

1. Opportunity signal intake (market pull, customer pain, founder thesis, ecosystem shift).
2. Job and outcome framing (what job is being done, how success is measured).
3. Problem space boundaries (what is in-scope, out-of-scope, and explicitly ignored now).
4. Constraint envelope (budget/time/team/tooling/compliance constraints for SMB reality).
5. Opportunity statement and risk assumptions (testable assumptions list with risk ranking).

### Stage 2: Problem and Segment Validation

1. Segment map and beachhead hypothesis (which segment to validate first and why).
2. Problem interviews and behavior evidence (past behavior, current workaround, urgency).
3. Pain quantification (frequency, impact, cost of inaction, switching friction).
4. Existing alternatives analysis (competitors, internal process, status-quo baseline).
5. Segment-problem fit verdict (validated / weak / rejected with explicit evidence quality).

### Stage 3: Solution and Offer Hypothesis

1. Value proposition design (pain relievers, gain creators, clear before/after value).
2. Offer architecture (core outcome, offer scope, service layer, promise wording).
3. Commercial model hypothesis (price metric, packaging, payment logic, willingness assumptions).
4. Channel and adoption hypothesis (where target buyers discover and how they decide).
5. Falsifiable test plan (clear thresholds for pass/fail before build investment).

### Stage 4: MVP or Proof-of-Value

1. MVP pattern selection (concierge, Wizard-of-Oz, prototype, fake door, thin-slice build).
2. Experiment design with instrumentation (events, conversion points, evidence collection plan).
3. Minimal implementation and assisted onboarding (manual where possible to reduce waste).
4. Real-user execution cycle (run experiments with beachhead users under realistic conditions).
5. Decision loop (pivot, refine, or persevere based on pre-agreed thresholds).

### Stage 5: Early Commercial Validation

1. Founder-led demand generation (direct outreach, warm intros, niche community entry).
2. Lead qualification and sales path mapping (who buys, who signs, cycle friction points).
3. Pricing and commitment proof (paid pilot, deposit, LOI quality, procurement feasibility).
4. First-customer success and references (time-to-value, first wins, reference readiness).
5. Repeatability check to first 10 customers (early pattern stability, not random wins).

### Stage 6: Fit Consolidation (PMF-Level Evidence)

1. Activation hardening (fast first value, onboarding reliability, usage quality).
2. Retention and cohort stabilization (repeat usage patterns, churn reasons, cohort shape).
3. Revenue quality validation (expansion readiness, NRR direction, downgrade patterns).
4. PMF confidence signals (qualitative pull + quantitative stickiness in same segment).
5. Failure closure loop (top churn causes and adoption blockers systematically removed).

### Stage 7: GTM and Route-to-Market Fit

1. ICP and positioning lock (who we target now, who we explicitly do not target now).
2. GTM motion selection (PLG, sales-led, or hybrid with clear ownership model).
3. Channel fit validation (one to two winning channels with predictable performance).
4. RTM architecture design (direct, partner, hybrid; account ownership and lead routing).
5. Unit economics readiness (CAC logic, payback direction, gross margin viability by segment).

### Stage 8: First Scalable Actions

1. Working motion codification (playbooks, scripts, qualification rules, handoff rules).
2. Controlled volume expansion (increase throughput only in proven channel-motion pairs).
3. Team and ownership scaling (who owns demand, conversion, onboarding, retention loops).
4. Continuous optimization system (weekly experiment cadence with shared scorecards).
5. Risk and quality guardrails (channel conflict rules, pricing boundaries, service consistency).

## Dry Run Before Level 2

Pre-implementation sanity check completed before adding deeper decomposition:

- No conflict with non-gated principle: gates below are local quality checks, not stage locks.
- No parallel map created: decomposition stays on the same axis and deepens current structure.
- Fail-fast preserved: each unit has explicit invalidation/failure triggers.
- YAGNI preserved: only evidence-critical artifacts are added.
- Doer visibility emerges naturally from bounded units, not from a separate doer table.

## Level 2 Decomposition (Executable Work Units)

This level describes how each Level 1 substage is executed in a reliability-first SMB context.
Each unit has: inputs, execution actions, output artifact, local evidence gate, fail-fast trigger.
...
## Level 2 Extraction Rule (How Doers Become Visible)

At Level 2, a doer-candidate emerges when a unit has all five attributes:

- one bounded decision class;
- one output artifact;
- one tool and policy boundary;
- one local evidence gate;
- one explicit fail-fast trigger.

Units missing one or more attributes need another decomposition pass before being promoted to expert candidates.

## Dry Run Before Level 3

Pre-implementation dry run completed before promoting units into experts:

- Scope integrity check: Level 3 stays on the same decomposition axis (no parallel map).
- Non-gating check: experts support boss-driven orchestration; no forced stage lock.
- Reliability check: every expert card includes explicit done-definition and failure path.
- Source integrity check: every non-user input must declare provenance (artifact/API/expert/tool output).
- YAGNI check: only experts with distinct output contract or tool-policy are introduced.

## Level 3 Decomposition (Expert Cards / Doer Layer)

### Level 3 Input Provenance Contract (Mandatory)

For every expert task input, declare source in this format:

- `input_field`: `<value or reference>`
- `input_source_type`: one of `user_directive`, `artifact`, `api`, `event_stream`, `expert_handoff`, `tool_output`
- `input_source_ref`: concrete reference (artifact name, API endpoint group, expert name, tool result id)
- `freshness_rule`: how current the input must be

No input is valid for execution without provenance declaration.
...
- Tool policy: allow research tools and document tooling; block execution-heavy delivery systems.
...
- Tool policy: allow survey/interview systems and note repositories; block pricing/ad spend actions.
...
- Tool policy: allow analysis and decision docs; block campaign execution tools.
...
- Tool policy: allow analytics/CRM reads; block outbound/customer messaging systems.
...
- Tool policy: allow research/doc tools and message testing prep; block media buying APIs.
...
- Tool policy: allow pricing/survey and commitment evidence reads; block ad operations tools.
...
- Tool policy: allow planning and analytics tooling; block production rollout systems.
...
- Tool policy: allow product analytics and prototype systems; block unrelated channel spend tools.
...
- Tool policy: allow CRM/outreach systems; block pricing governance changes.
...
- Tool policy: allow creative tools and ad-platform APIs under spend cap; block unapproved budget expansion.
...
- Tool policy: allow CRM/onboarding/analytics systems; block channel architecture changes.
...
- Tool policy: allow analytics, billing, survey reads; block direct campaign execution.
...
- Tool policy: allow CRM/support/survey reads and interview tooling; block pricing changes.
...
- Tool policy: allow finance/CRM/channel analytics and partner-policy docs; block direct content generation.
...
- Tool policy: allow PRM/partner operations tools and governance docs; block direct pricing overrides.
...
- Tool policy: allow ops/governance tooling and KPI systems; block ungoverned process exceptions.
