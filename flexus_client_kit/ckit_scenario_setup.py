...
class ScenarioSetup:
    ...
    async def create_group_hire_and_start_bot(
        ...
        self.ws = await select_workspace_for_scenario(self.fclient, self.bs, marketable_name)
        await self.create_test_group(group_prefix)
        await self.hire_bot(marketable_name, marketable_version, persona_setup)
    ...
    async def run_scenario(self, scenario_func: Callable[..., None], cleanup_wait_secs: int = 300, **kwargs) -> None:
        ...
        try:
            ...
            personas_output = await self.print_personas_in_group()
            print(personas_output)
            threads_output = await self.print_threads_in_group()
