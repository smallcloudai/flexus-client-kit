...
logger = logging.getLogger("newsdata")
...
class IntegrationNewsdata:
    ...
    async def _dispatch(self, method_id: str, args: Dict[str, Any]) -> str:
        if method_id == "newsdata.news.search.v1":
    ...
    async def _news_search(self, args: Dict[str, Any]) -> str:
        api_key = self._get_api_key()
        if not api_key:
            return json.dumps({"ok": False, "error_code": "AUTH_MISSING", "message": "Set NEWSDATA_KEY env var."}, indent=2, ensure_ascii=False)
