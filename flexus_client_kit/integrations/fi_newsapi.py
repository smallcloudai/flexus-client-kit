...
class IntegrationNewsapi:
    ...
    def _get_api_key(self) -> str:
        if self.rcx is not None:
            return (self.rcx.external_auth.get("newsapi") or {}).get("api_key", "")
    ...
    async def _dispatch(self, method_id: str, args: Dict[str, Any]) -> str:
        if method_id == "newsapi.everything.v1":
            return await self._everything(args)
    ...
    async def _everything(self, args: Dict[str, Any]) -> str:
        api_key = self._get_api_key()
        if not api_key:
            return json.dumps({"ok": False, "error_code": "AUTH_MISSING", "message": "Set NEWSAPI_KEY env var."}, indent=2, ensure_ascii=False)
