...
class IntegrationGnews:
    def __init__(self, rcx=None):
        self.rcx = rcx

    def _get_api_key(self) -> str:
        if self.rcx is not None:
            return (self.rcx.external_auth.get("gnews") or {}).get("api_key", "")
    ...
    async def _dispatch(self, method_id: str, args: Dict[str, Any]) -> str:
        if method_id == "gnews.search.v1":
            return await self._search(args)
        if method_id == "gnews.top_headlines.v1":
            return await self._top_headlines(args)
    ...
    async def _search(self, args: Dict[str, Any]) -> str:
        api_key = self._get_api_key()
        if not api_key:
            return json.dumps({"ok": False, "error_code": "AUTH_MISSING", "message": "Set GNEWS_KEY env var."}, indent=2, ensure_ascii=False)
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                r = await client.get(_BASE_URL + "/search", params=params, headers={"Accept": "application/json"})
            ...
            total = data.get("totalArticles", len(articles))
    ...
    async def _top_headlines(self, args: Dict[str, Any]) -> str:
        api_key = self._get_api_key()
        if not api_key:
            return json.dumps({"ok": False, "error_code": "AUTH_MISSING", "message": "Set GNEWS_KEY env var."}, indent=2, ensure_ascii=False)
        limit = min(int(args.get("limit", 10)), 10)
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                r = await client.get(_BASE_URL + "/top-headlines", params=params, headers={"Accept": "application/json"})
            if r.status_code >= 400:
                logger.info("%s HTTP %s: %s", PROVIDER_NAME, r.status_code, r.text[:200])
                return json.dumps({"ok": False, "error_code": "PROVIDER_ERROR", "status": r.status_code, "detail": r.text[:300]}, indent=2, ensure_ascii=False)
            data = r.json()
            ...
            if include_raw:
                result["raw"] = data
            summary = f"Found {len(articles)} headline(s) from {PROVIDER_NAME} (total={total})."
            return summary + "\n\n```json\n" + json.dumps(result, indent=2, ensure_ascii=False) + "\n```"
        except httpx.TimeoutException:
            return json.dumps({"ok": False, "error_code": "TIMEOUT", "provider": PROVIDER_NAME}, indent=2, ensure_ascii=False)
        except (httpx.HTTPError, ValueError, KeyError, json.JSONDecodeError) as e:
