"""
Facebook Creative & Ads Operations

Operations for creating ad creatives and ads.
Creatives contain the actual ad content (images, text, links).
"""
...
async def upload_image(
    ...
    Provide either image_path (local file) or image_url (remote URL).
    ...
    Returns:
        Formatted result string with image hash
    """
...
"""
    ...
    if image_url:
        ...
        result = await client.post(endpoint, form_data=form_data)
        ...
        # For file uploads, we need to use httpx directly with files parameter
...
async def create_creative(
    ...
    Returns:
        Formatted result string with creative ID
    """
    if not name:
...
"""
    ...
    if headline:
        link_data["name"] = headline  # Facebook uses 'name' for headline
    if description:
    ...
    result = await client.post(f"{account_id}/adcreatives", data=data)
...
async def create_ad(
    ...
    Returns:
        Formatted result string with ad ID
    """
    if not name:
        return "ERROR: name is required"
    if not adset_id:
...
"""
    ...
    result = await client.post(f"{account_id}/ads", data=data)
...
async def preview_ad(
    ...
    Args:
        client: Authenticated Facebook client
        ad_id: Ad ID
        ad_format: Preview format (default DESKTOP_FEED_STANDARD)

    Returns:
        Formatted preview result
    """
    if not ad_id:
