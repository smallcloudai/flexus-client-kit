...
from .accounts import list_ad_accounts, get_ad_account_info, update_spending_limit
from .campaigns import list_campaigns, create_campaign, update_campaign, duplicate_campaign, archive_campaign, bulk_update_campaigns, get_insights
from .adsets import list_adsets, create_adset, update_adset, validate_targeting
from .ads import upload_image, create_creative, create_ad, preview_ad
if TYPE_CHECKING:
...
_OPERATION_HANDLERS = {
    ...
    "get_ad_account_info": lambda client, args: get_ad_account_info(
        client, args.get("ad_account_id", "")
    ),
    "update_spending_limit": lambda client, args: update_spending_limit(
        client, args.get("ad_account_id", ""), args.get("spending_limit", 0)
    ),
    "list_campaigns": lambda client, args: list_campaigns(
    ...
    "duplicate_campaign": lambda client, args: duplicate_campaign(
        client, args.get("campaign_id", ""), args.get("new_name", ""), args.get("ad_account_id")
    ),
    "archive_campaign": lambda client, args: archive_campaign(
        client, args.get("campaign_id", "")
    ),
    "bulk_update_campaigns": lambda client, args: bulk_update_campaigns(
        client, args.get("campaigns", [])
    ),
    "get_insights": lambda client, args: get_insights(
        client, args.get("campaign_id", ""), int(args.get("days", 30))
    ),
    "list_adsets": lambda client, args: list_adsets(
        client, args.get("campaign_id", "")
    ),
    ...
    "validate_targeting": lambda client, args: validate_targeting(
        client, args.get("targeting_spec", args.get("targeting", {})), args.get("ad_account_id")
    ),
    "upload_image": lambda client, args: upload_image(
    ...
    "preview_ad": lambda client, args: preview_ad(
...
class IntegrationFacebook:
    ...
    async def called_by_model(
        ...
        if not handler:
            return f"Unknown operation '{op}'. Try op=\"help\" for available "
        try:
...
"""
        ...
        campaigns_result = await list_campaigns(
