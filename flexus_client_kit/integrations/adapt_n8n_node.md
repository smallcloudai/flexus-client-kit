Adapt the following n8n node to this flexus repo, take a look at another n8n adapted integrations as inspiration, like @flexus_client_kit/integrations/fi_gmail.py
...
1. If there is just one tool, leave it as it is, if there is more, then do like fi_gmail.py and compress them in one using op and args.
2. If the auth is with oauth2, do it like in fi_gmail.py so user has to click the link to give permission, if it is api key or similar variable, add it in the bot setup, if it only needs one api key from our side (flexus, the system), just use the env var.
3. Ensure to follow rules in @AGENTS.md and follow the coding standards in this repo.
4. If the node involves calling the api, and the n8n node calls directly the api, consider using a python SDK if there is one available and good for the case, instead of raw querying the API.
...
The n8n node to adapt and the variables needed are:
