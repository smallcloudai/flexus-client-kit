...
logger = logging.getLogger("perigon")
...
_BASE_URL = "https://api.goperigon.com/v1"
...
class IntegrationPerigon:
    def __init__(self, rcx=None):
        self.rcx = rcx

    def _get_api_key(self) -> str:
        if self.rcx is not None:
            return (self.rcx.external_auth.get("perigon") or {}).get("api_key", "")
    ...
    async def _dispatch(self, method_id: str, args: Dict[str, Any]) -> str:
        if method_id == "perigon.all.search.v1":
            return await self._all_search(args)
        if method_id == "perigon.topics.search.v1":
            return await self._topics_search(args)
        return json.dumps({"ok": False, "error_code": "METHOD_UNIMPLEMENTED", "method_id": method_id}, indent=2, ensure_ascii=False)

    async def _all_search(self, args: Dict[str, Any]) -> str:
        api_key = self._get_api_key()
        if not api_key:
            return json.dumps({"ok": False, "error_code": "AUTH_MISSING", "message": "Set PERIGON_KEY env var."}, indent=2, ensure_ascii=False)
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                r = await client.get(_BASE_URL + "/all", params=params, headers={"Accept": "application/json"})
            ...
            total = data.get("numResults", data.get("total", len(articles)))
    ...
    async def _topics_search(self, args: Dict[str, Any]) -> str:
        api_key = self._get_api_key()
        if not api_key:
            return json.dumps({"ok": False, "error_code": "AUTH_MISSING", "message": "Set PERIGON_KEY env var."}, indent=2, ensure_ascii=False)
        query = str(args.get("query", "")).strip()
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                r = await client.get(_BASE_URL + "/topics", params=params, headers={"Accept": "application/json"})
            if r.status_code >= 400:
                logger.info("%s HTTP %s: %s", PROVIDER_NAME, r.status_code, r.text[:200])
                return json.dumps({"ok": False, "error_code": "PROVIDER_ERROR", "status": r.status_code, "detail": r.text[:300]}, indent=2, ensure_ascii=False)
            data = r.json()
            topics = data.get("topics", data.get("results", []))
            total = data.get("numResults", data.get("total", len(topics)))
            result: Dict[str, Any] = {"ok": True, "results": topics, "total": total}
            if include_raw:
                result["raw"] = data
            summary = f"Found {len(topics)} topic(s) from {PROVIDER_NAME} (total={total})."
            return summary + "\n\n```json\n" + json.dumps(result, indent=2, ensure_ascii=False) + "\n```"
        except httpx.TimeoutException:
            return json.dumps({"ok": False, "error_code": "TIMEOUT", "provider": PROVIDER_NAME}, indent=2, ensure_ascii=False)
        except (httpx.HTTPError, ValueError, KeyError, json.JSONDecodeError) as e:
