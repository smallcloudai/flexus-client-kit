...
logger = logging.getLogger("newscatcher")
...
_BASE_URL = "https://v3-api.newscatcherapi.com/api"
...
class IntegrationNewscatcher:
    def __init__(self, rcx=None):
        self.rcx = rcx

    def _get_api_key(self) -> str:
        if self.rcx is not None:
            return (self.rcx.external_auth.get("newscatcher") or {}).get("api_key", "")
    ...
    async def _dispatch(self, method_id: str, args: Dict[str, Any]) -> str:
        if method_id == "newscatcher.search.v1":
            return await self._search(args)
        if method_id == "newscatcher.latest_headlines.v1":
            return await self._latest_headlines(args)
    ...
    async def _search(self, args: Dict[str, Any]) -> str:
        api_key = self._get_api_key()
        if not api_key:
            return json.dumps({"ok": False, "error_code": "AUTH_MISSING", "message": "Set NEWSCATCHER_KEY env var."}, indent=2, ensure_ascii=False)
    ...
    async def _latest_headlines(self, args: Dict[str, Any]) -> str:
        api_key = self._get_api_key()
        if not api_key:
            return json.dumps({"ok": False, "error_code": "AUTH_MISSING", "message": "Set NEWSCATCHER_KEY env var."}, indent=2, ensure_ascii=False)
        limit = min(int(args.get("limit", 25)), 100)
