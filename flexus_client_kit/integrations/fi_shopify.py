...
shopify(op="create_draft_order", args={
    ...
    "email": "customer@example.com", "note": "VIP order",
})
...
For all write ops, shop_id is optional when only one shop is connected."""
...
class IntegrationShopify:
    ...
    async def called_by_model(self, toolcall: ckit_cloudtool.FCloudtoolCall, mpa: Optional[Dict[str, Any]]) -> str:
        if not mpa:
            return HELP
        op = mpa.get("op", "")
