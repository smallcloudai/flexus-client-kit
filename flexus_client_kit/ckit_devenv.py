...
async def dev_environment_delete(http: gql.Client, devenv_id: str, fuser_id: str) -> None:
    await http.execute(
...
async def dev_environment_get_github_auth_url(http: gql.Client, devenv_id: str) -> str:
...
async def format_devenv_list(http: gql.Client, fgroup_id: str) -> str:
    devenv_list = await dev_environments_list_in_subgroups(http, fgroup_id)
    if devenv_list:
