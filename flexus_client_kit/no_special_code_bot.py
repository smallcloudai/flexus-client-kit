...
async def install_from_manifest(m, setup_schema, bot_dir, client, bot_name, bot_version, tools):
    ...
    for rec in ckit_integrations_db.integrations_load(m["integrations"], bot_dir=bot_dir):
...
async def bot_main_loop(m, setup_schema, bot_dir, fclient: ckit_client.FlexusClient, rcx: ckit_bot_exec.RobotContext) -> None:
    ...
    await ckit_integrations_db.integrations_init_all(ckit_integrations_db.integrations_load(m["integrations"], bot_dir=bot_dir), rcx)
...
def main():
    ...
    integrations = ckit_integrations_db.integrations_load(manifest["integrations"], bot_dir=bot_dir)
