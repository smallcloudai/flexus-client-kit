...
def format_binary_output(
    ...
    if b'\x00' in data[:1000]:
        ...
        result += "💡 Use 'download' operation to retrieve this file"
    else:
        try:
...
async def run_jq_query(
    ...
    if file_json == {}:
        return f"Error: File {path} has no content"
    json_input = json.dumps(file_json)
    cmd = ["jq"] + jq_args
    ...
    safety_valve_kb = int(safety_valve[:-1]) if safety_valve.lower().endswith('k') else 50
    size_bytes = len(content_str.encode('utf-8'))
    size_kb = size_bytes / 1024
    header_lines = [
        f"📄 File: {path}",
    ...
    # XXX : line no.
    if truncated:
        result += f"\n\n To see full content, increase the safety_valve"
