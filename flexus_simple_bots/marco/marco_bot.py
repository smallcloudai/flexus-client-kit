...
logger = logging.getLogger("bot_marco")

MARCO_ROOTDIR = Path(__file__).parent
MARCO_SKILLS = ckit_skills.static_skills_find(MARCO_ROOTDIR, shared_skills_allowlist="*", integration_skills_allowlist="*")

MARCO_SETUP_SCHEMA = json.loads((MARCO_ROOTDIR / "setup_schema.json").read_text())
MARCO_SETUP_SCHEMA += (
...
MARCO_INTEGRATIONS: list[ckit_integrations_db.IntegrationRecord] = ckit_integrations_db.static_integrations_load(
    MARCO_ROOTDIR,
    allowlist=[
        "skills",
...
TOOLS = [
    ...
    *[t for rec in MARCO_INTEGRATIONS for t in rec.integr_tools],
]


async def marco_main_loop(fclient: ckit_client.FlexusClient, rcx: ckit_bot_exec.RobotContext) -> None:
    setup = ckit_bot_exec.official_setup_mixing_procedure(MARCO_SETUP_SCHEMA, rcx.persona.persona_setup)

    integrations = await ckit_integrations_db.main_loop_integrations_init(MARCO_INTEGRATIONS, rcx, setup)
    ...
    async def slack_activity_callback(a: fi_slack.ActivitySlack, already_posted: bool):
        ...
        if a.message_author_id:
            if contact_id := await fi_crm.find_contact_by_platform_id(await fclient.use_http_on_behalf(rcx.persona.persona_id, ""), rcx.persona.ws_id, "slack", a.message_author_id):
                extra["contact_id"] = contact_id
        await slack.inbound_activity_to_task(a, already_posted_to_captured_thread=False, extra_details=extra, provenance="marco_slack_activity")
...
def main():
    from flexus_simple_bots.marco import marco_install
    ...
    asyncio.run(ckit_bot_exec.run_bots_in_this_group(
        fclient,
        bot_main_loop=marco_main_loop,
        inprocess_tools=TOOLS,
        scenario_fn=scenario_fn,
        install_func=marco_install.install,
