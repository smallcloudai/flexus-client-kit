...
2. Click the "Upload Documents" button or drag files in
...
If the user wants to add content from a website, use eds_setup to create a crawler data source:

```python
eds_setup(op="help", args={"eds_type": "Web Crawler"})
eds_setup(op="create", args={"eds_name": "Example Docs", "eds_type": "crawler", "eds_json": {"start_url": "https://example.com", "max_depth": 1, "max_documents": 100, "rescan_period": 86400}})
```

- `max_depth=0`: single page only
- `max_depth=1`: page + all linked pages on the same domain
