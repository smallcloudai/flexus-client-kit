...
1. **Preflight and scope lock**
   ...
   - Call `linkedin_b2b(op="status")` first.
...
2. **Collect organization post evidence**
   - For each organization, retrieve recent posts via `linkedin_b2b.organization_posts.list.v1`.
...
3. **Collect engagement quality evidence**
   - Pull post-level social metadata via `linkedin_b2b.social_metadata.get.v1`.
...
4. **Collect follower momentum evidence**
   - Pull follower stats via `linkedin_b2b.followers.stats.get.v1`.
...
Use only verified connector methods. Do not invent method IDs. If uncertain, call `linkedin_b2b(op="help")` first and work only with listed methods.

```python
linkedin_b2b(op="status")
linkedin_b2b(op="help")
...
linkedin_b2b(
  op="call",
  args={
    "method_id": "linkedin_b2b.social_metadata.get.v1",
...
linkedin_b2b(
  op="call",
  args={
    "method_id": "linkedin_b2b.followers.stats.get.v1",
    "organization_id": "12345"
  }
)
```
...
1. You ran `linkedin_b2b(op="status")` and handled auth state correctly.
