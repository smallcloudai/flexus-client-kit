...
from flexus_simple_bots.handcrafted.productman.productman_prompts import PRODUCTMAN_BASE
...
2. Draft survey and audience targeting:
   ...
   - Create survey draft: `survey(op="draft_survey", args={{"idea_slug": "dental-samples", "hypothesis_slug": "private-practice", "survey_content": {{...}}}})`
   - Create audience draft: `survey(op="draft_auditory", args={{"idea_slug": "dental-samples", "hypothesis_slug": "private-practice", "study_name": "...", "estimated_minutes": 5, "reward_cents": 200, "total_participants": 30, "filters": {{...}}}})`
   - If no specific filter exists, use screening questions in section01
   - Show cost estimates and get user approval

3. Execute campaign:
   - Call `survey(op="run", args={{"idea_slug": "dental-samples", "hypothesis_slug": "private-practice"}})`
