"""
Facebook Campaign Management (Extended Operations)

WHAT: Extended campaign operations beyond basic CRUD.
WHY: Enables campaign optimization, duplication, and bulk operations.

OPERATIONS:
- update_campaign: Modify campaign name, status, or budget
- duplicate_campaign: Copy a campaign with new name (created PAUSED)
- archive_campaign: Soft-delete by setting status to ARCHIVED
- bulk_update_campaigns: Update multiple campaigns in one call
...
async def handle(integration, toolcall, model_produced_args: Dict[str, Any]) -> str:
    """
    Router for campaign operations. Catches all exceptions.
    ...
    try:
        ...
        logger.info(f"Facebook API error in campaign: {e}")
        return f"❌ Facebook API Error: {e.message}"
    except Exception as e:
        logger.warning(f"Campaign error: {e}", exc_info=e)
...
async def duplicate_campaign(integration, args: Dict[str, Any]) -> str:
    """Create a copy of an existing campaign (new campaign starts PAUSED)."""
    campaign_id = args.get("campaign_id", "")
...
async def bulk_update_campaigns(integration, args: Dict[str, Any]) -> str:
    """Update multiple campaigns at once (max 50)."""
