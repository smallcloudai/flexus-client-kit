...
The model calls it with up to 6 questions. Format per question: `"question text | type | option1, option2, ..."`. Types: `single` (pick one), `multi` (pick many), `yesno`, `text` (free input). The handler returns `⏸️WAIT_FOR_USER` which pauses the chat until the user responds.
